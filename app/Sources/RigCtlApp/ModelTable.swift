...
enum ModelTable {
    ...
    private static var models: [RigModel] = []
    private static var loaded = false

    static func name(for id: Int) -> String {
        load()
        return cache[id] ?? "model \(id)"
    }

    static var all: [RigModel] {
        load()
        return models
    }

    static func model(withID id: Int) -> RigModel? {
        load()
    ...
    static func id(fromListing text: String) -> Int? {
        let trimmed = text.trimmingCharacters(in: .whitespaces)
        if let n = Int(trimmed) { return model(withID: n) != nil ? n : nil }
        load()
    ...
    static func suggestions(forProduct product: String?) -> [RigModel] {
        load()
    ...
    private static func load() {
        ...
        for line in text.split(separator: "\n") {
            ...
            cache[id] = label
            models.append(RigModel(id: id, manufacturer: cols[0], model: cols[1],
